...
bool sdSelectCard(uint8_t pdrv)
{
    ...
    if (!s)
    {
        ESP_LOGE("sd_diskio", "Select Failed");
        digitalWrite(card->ssPin, HIGH);
        return false;
    }
    return true;
...
char sdCommand(uint8_t pdrv, char cmd, unsigned int arg, unsigned int *resp)
{
    ...
    if (token == 0xFF)
    {
        ESP_LOGE("sd_diskio", "Card Failed! cmd: 0x%02x", cmd);
...
bool sdReadSectors(uint8_t pdrv, char *buffer, unsigned long long sector, int count)
{
    for (int f = 0; f < 3;)
    {
        ...
        if (!sdCommand(pdrv, READ_BLOCK_MULTIPLE, (s_cards[pdrv]->type == CARD_SDHC) ? sector : sector << 9, NULL))
        {
            ...
            {
                ESP_LOGE("sd_diskio", "command failed");
                break;
            }
...
bool sdcard_mount(uint8_t pdrv, const char *path, uint8_t max_files, bool format_if_empty)
{
    ...
    {
        ESP_LOGE("sd_diskio", "esp_vfs_fat_register failed 0x(%x): SD is registered.", err);
        return false;
    }
    else if (err != ESP_OK)
    {
        ESP_LOGE("sd_diskio", "esp_vfs_fat_register failed 0x(%x)", err);
        return false;
    }
    ...
    if (res != FR_OK)
    {
        ESP_LOGE("sd_diskio", "f_mount failed: %s", fferr2str[res]);
        if (res == 13 && format_if_empty)
        {
            ...
            if (res != FR_OK)
            {
                ESP_LOGE("sd_diskio", "f_mkfs failed: %s", fferr2str[res]);
